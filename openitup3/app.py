...
def code_visualizer():
    ...
    if request.method == 'POST':
        if not code_to_visualize.strip():
            result = "Please paste a function to visualize."
        else:
            prompt = f"""
            ...
            - Provide ONLY the Mermaid syntax inside a single mermaid code block.

            Python Code:
            ```{code_to_visualize}```
            """
            # We call the model and then strip the markdown code block fences
            raw_result = call_ollama(prompt)
            result = raw_result.replace("```mermaid", "").replace("```", "").strip()
